...
def standard_performance(loc:str, jar:str, area:float=0.5, **peis_args):
    ...
    ohmic_rtot = [round(ohmic,2),round(rp,2)+round(ohmic,2)]
    plot_peis(area,loc,ohmic_rtot=ohmic_rtot,**peis_args) #Plots standard PEIS spectra
    plt.show()
    print('Standard Ohmic',round(ohmic,3),'\u03A9*cm$^2^')
    print('Standard Rp',round(rp,3),'\u03A9*cm$^2^')

def po2_plots(folder_loc:str, fit_path:str, area:float, eis:bool=True,
                drt:bool=True, o2_dependence:bool=True, print_resistance_values:bool=False,
                ncol:int=1, legend_loc:str='best'):
    '''
    ...
    param folder_loc, str: The location of the folder containing the EIS files (path to folder)
    param fit_path, str: The location of the folder to save the map-fits (path to folder)
    param area, float: The active cell area in cm^2
    param eis, bool: If True, plots the EIS for each concentration in one plot (default = True)
    param drt, bool: If True, plots the DRT for each concentration in another plot (default = True)
    param o2_dependence, bool: If True, plots the ln(1/ASR) vs. ln(O2 concnentration) for the ohmic and polarization resistance (default = True)
    param print_resistance_values, bool: If True, prints the ohmic and polarization resistance for each concentration (default = False)
    param ncol, int: The number of columns to use for the plot legend (default = 1)
...
def fc_bias_plots(folder_loc:str, fit_path:str, area:float, eis:bool=True, drt:bool=True,
    ...
    param drt_peaks, bool: If True, the DRT peaks are fit and plotted
    ...
    if eis == True:
        ...
        cmap = plt.cm.get_cmap('turbo') #cmr.redshift 
...
def deg_ocv_eis_plots(folder_loc:str, jar_loc:str, area:float, start_file:str = 'default',
    ...
    cell_name = os.path.basename(folder_loc).split("_", 3)[2]
    if a10 == True: # I need to find the start time of the test
